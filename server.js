...
const { ALLOWED_FOLDERS, testConnection, scanFolders } = require('./imap-readonly');
...
CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY, insurer_id INTEGER NOT NULL, subject TEXT NOT NULL, category TEXT NOT NULL, received_at TEXT NOT NULL, valid_from TEXT, valid_to TEXT, sender TEXT, source_mailbox TEXT, body_html TEXT DEFAULT '', body_text TEXT DEFAULT '', source_message_id TEXT UNIQUE, created_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(insurer_id) REFERENCES insurers(id));
CREATE TABLE IF NOT EXISTS items (id INTEGER PRIMARY KEY, message_id INTEGER NOT NULL, title TEXT NOT NULL, category TEXT NOT NULL, valid_from TEXT, valid_to TEXT, status_override TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(message_id) REFERENCES messages(id) ON DELETE CASCADE);
...
function seed(){
 ...
 const examples=[['PZU','Niedostępność aplikacji Kandydat – prace serwisowe','Systemy','2026-09-14','2026-09-14','2026-09-14','PZU','Outlook','Prace serwisowe. Niedostępność aplikacji Kandydat w godzinach 17:00–19:00.','demo-pzu-1',['Kandydat','prace serwisowe']],['Compensa','Letnie FLOW 3','Konkursy i akcje','2026-09-01','2026-09-01','2026-09-30','Compensa','LH','Konkurs Letnie FLOW 3. Szczegóły w regulaminie.','demo-comp-1',['FLOW','konkurs']],['Warta','15% zniżki na nowe domy i mieszkania','Promocje i zniżki','2026-09-15','2026-09-15','2026-09-16','Warta','LH','Promocja 15% na nowe domy i mieszkania.','demo-warta-1',['dom','zniżka']],['InterRisk','Brak przeglądów – Dom Max i PM+ wariant Max w Iron','Produkty','2026-09-10','2026-09-10',null,'InterRisk','LH','Informacja produktowa dotycząca wymogu przeglądów.','demo-ir-1',['Dom Max','Iron']],['PZU','Co nowego w majątku?','Produkty','2026-09-12','2026-09-12',null,'PZU','Outlook','Newsletter źródłowy zawierający kilka tematów.','demo-pzu-2',['newsletter']]];
...
// IMAP preview endpoints. Password is used only for this request and is never written to SQLite or disk.
app.get('/api/imap/config',(req,res)=>res.json({address:'kontakt@elefi.pl',host:'mail-serwer287110.lh.pl',port:993,secure:true,readOnly:true,folders:ALLOWED_FOLDERS}));
app.post('/api/imap/test',async(req,res)=>{try{const folders=await testConnection(String(req.body.password||''));res.json({ok:true,readOnly:true,folders})}catch(e){res.status(400).json({ok:false,error:e.message})}});
app.post('/api/imap/scan',async(req,res)=>{try{const folders=await scanFolders(String(req.body.password||''));res.json({ok:true,readOnly:true,folders,total:folders.reduce((n,x)=>n+x.messages,0)})}catch(e){res.status(400).json({ok:false,error:e.message})}});
